...
let allOrdersIds = userData.cart.map((orderId) => +orderId);
console.log(allOrdersIds);
let cart = products.filter((order) => allOrdersIds.includes(order.id));
const container = document.getElementById("cart");

cart.length != 0
  ? (container.innerHTML = "")
  : (container.innerHTML = `<div class="w-100 mx-auto mt-4 text-center">
          <h4 class="my-2 ">You haven't uploaded any orders yet</h4>
...
function deleteProduct(index) {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      location.href = "/customer/cart/index.html"; // simplest way to re-render
