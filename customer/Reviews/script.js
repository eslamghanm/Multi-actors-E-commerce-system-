...
const keys = Object.keys(userData.ratings);
console.log(keys);
// console.log(userData.ratings.keys());
let allOrdersIds = keys.map((orderId) => +orderId);
...
function deleteProduct(index) {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      delete userData.ratings[keyToDelete];
      if (userData !== -1) {
      ...
      location.href = "/customer/Reviews/index.html"; // simplest way to re-render
